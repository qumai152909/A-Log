...
+ http://120.27.215.50/scc/imp/js/app.js/  ； 返回/webApp/redirect.html
...
server {
  ...
  error_log                /webApp/error.log warn;
	...
	location /api {
	    proxy_pass http://127.0.0.1:80/;  # 1，结尾有斜杠、替换操作
      proxy_pass http://127.0.0.1:80;   # 2，结尾无斜杠、追加操作
...
~~~nginx
    server {
        ...
        rewrite "^/scc/imp/(.*)$" /$1 break;
  			...
  			# 注意，此处需写为/api/ ，如果写为/api，则出现错误，具体见下方
        location /api/ {
...
*访问：http://120.27.215.50/api/ 代理返回：https://www.baidu.com/页面内容
...
好像是 http://120.27.215.50/scc/imp/api通过rewrite和proxy_pass https://www.baidu.com/代理到了https://www.baidu.com/api

不应该是https://www.baidu.com/吗？
